...
- 강사(이한영) 보충 설명
```
	- 동일한 메서드를 해당 클래스로 만들어진 객체(인스턴스)들에서 모두 사용가능하다.
	- 이때 self로 사용하는 인스턴스의 정보가 넘어오기 때문에 같은 메서드이지만 각 인스턴스마다 다른 동작을 해줄수 있다.
	- 그래서 하나의 메서드를 여러 객체(인스턴스)가 공유할 수 있다. 라고 말한다.
	- 인스턴스 메서드에서 self로 주어지는 인자는 항상 해당 클래스형 객체이다.
```
> **실습**
>	
...
- 강사(이한영) 보충 설명
```
	- 특정 메서드가 다른 타입(형)의 객체가 주어짐에 따라 다른 동작을 하는것이 다형성.
```
```python
# 다형성 예제
class User :
